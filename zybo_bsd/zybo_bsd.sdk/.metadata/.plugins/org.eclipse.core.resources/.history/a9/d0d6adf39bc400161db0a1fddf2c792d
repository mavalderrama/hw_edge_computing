...
CPU_BOOLEAN  HTTPcApp_ReqSendPost (void)
{
    ...
    //p_data = "{\"zyb\":111}";
    //strcpy(&HTTPcApp_FormBuf[0],"{\"zyb\":123}");
    ...
    if (result == DEF_OK) {
        HTTPc_APP_TRACE_INFO(("Connection to server UBIDOTS succeeded.\n\r"));
    } else {
        HTTPc_APP_TRACE_INFO(("Connection to server failed.\n\r"));
        return (DEF_FAIL);
    }
    ...
    xil_printf("reqsend\n");                                                            /* ---------------- SEND HTTP REQUEST ----------------- */
    str_len = Str_Len("/api/v1.6/devices/TestZybo");
    flags   = HTTPc_FLAG_NONE;
    result  = HTTPc_ReqSend(p_conn,
                            p_req,
                            p_resp,
                            HTTP_METHOD_POST,
                           "/api/v1.6/devices/TestZybo",
                            str_len,
                            flags,
                           &err);
    if (err != HTTPc_ERR_NONE) {
...
CPU_BOOLEAN  HTTPcAPP_ReqSendPut (void)
{
    ...
    if (result == DEF_OK) {
        HTTPc_APP_TRACE_INFO(("Connection to server ubidots bitch!! succeeded.\n\r"));
    } else {
        HTTPc_APP_TRACE_INFO(("Connection to server failed.\n\r"));
        return (DEF_FAIL);
    }
...
static  CPU_INT08U  HTTPcApp_ReqPrepareFormApp (HTTPc_FORM_TBL_FIELD  **p_form_tbl)
{
    ...
    p_kvp->KeyLen = Str_Len("{\"test\":123}");
    //p_kvp->ValLen = Str_Len("");

   (void)Str_Copy_N(p_kvp->KeyPtr,"{\"test", p_kvp->KeyLen);
   (void)Str_Copy_N(p_kvp->ValPtr,"\":123}",  p_kvp->ValLen);
